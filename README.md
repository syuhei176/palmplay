...
- **[cockpit.html](cockpit.html) — Hand Cockpit ✈️**: コックピット視点の3Dフライト（Three.js）。✊左手＝スロットルレバー（上げて加速）、✊右手＝操縦桿（手前に引く＝機首上げ／左右に振る＝バンク旋回）。Vrまで加速→ROTATEで離陸、その後は**自由飛行**で空を飛び回れる。失速・墜落注意。カメラ無しでもキーボード（W/S・↑/↓・←/→）で操作可。
